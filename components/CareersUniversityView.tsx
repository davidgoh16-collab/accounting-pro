...
import { GeographyCareer, UniversityCourseInfo, TransferableSkill, CVSuggestions } from '../types';
import { generateCareerInfo, generateUniversityCourseInfo, generateTransferableSkillInfo, generateCVSuggestions, generateTopUKUniversityInfo } from '../services/geminiService';
...
// ... (existing constants and components remain unchanged: CAREER_CATEGORIES, TRANSFERABLE_SKILLS, LoadingSpinner, ErrorDisplay, SkillModal, CareersExplorer, UniversityFinder, SkillsSpotlight, CVBuilder)
...
type ActiveTab = 'careers' | 'university' | 'skills' | 'cv_builder';
...
const UniversityFinder: React.FC = () => {
    ...
    const handleSearch = (e: React.FormEvent) => {
        ...
        setCourses([]);
        setSources([]);
        generateUniversityCourseInfo(interests)
    ...
    return (
        <div>
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-center gap-3 mb-6 p-4 bg-white/50 backdrop-blur-sm border rounded-2xl">
                ...
                />
                <button type="submit" disabled={isLoading || !interests.trim()} className="w-full sm:w-auto px-6 py-2 bg-rose-500 text-white font-bold rounded-lg hover:bg-rose-600 transition disabled:bg-stone-400">
                    Find Courses
                </button>
                 <button type="button" onClick={handleTopUnis} disabled={isLoading} className="w-full sm:w-auto px-6 py-2 bg-stone-500 text-white font-bold rounded-lg hover:bg-stone-600 transition disabled:bg-stone-400">
                    Top 5 UK Unis
                </button>
            </form>
...
const CareersUniversityView: React.FC<CareersUniversityViewProps> = ({ onBack }) => {
    ...
    const TabButton: React.FC<{ tab: ActiveTab; label: string }> = ({ tab, label }) => (
        <button
            ...
            className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-lg focus:outline-none ${activeTab === tab ? 'border-rose-500 text-rose-600' : 'border-transparent text-stone-500 hover:text-stone-700 hover:border-stone-300'}`}
        >
            {label}
        </button>
    );
