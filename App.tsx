...
import { useGameTimeLimit } from './components/useGameTimeLimit';
import GameTimerBanner from './components/GameTimerBanner';
...
const CountdownWidget: React.FC<{ mocks: MockConfig[], userLevel?: UserLevel, userYearGroup?: string }> = ({ mocks, userLevel, userYearGroup }) => {
    const nextExam = useMemo(() => {
        ...
        const now = new Date();
        const futureExams = allExams.filter(e => new Date(`${e.date}T${e.time || '09:00'}:00`) > now);
        futureExams.sort((a, b) => new Date(`${a.date}T${a.time || '09:00'}:00`).getTime() - new Date(`${b.date}T${b.time || '09:00'}:00`).getTime());
    ...
    const calculateTimeLeft = () => {
        ...
        if (difference > 0) return { d: Math.floor(difference / (1000 * 60 * 60 * 24)), h: Math.floor((difference / (1000 * 60 * 60)) % 24), m: Math.floor((difference / 1000 / 60) % 60), s: Math.floor((difference / 1000) % 60) };
        return {};
    };
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            try {
                ...
                setHistory(querySnapshot.docs.map(doc => doc.data() as CompletedSession));
            } catch (error) { console.error("Could not load session history from Firestore", error); }
        };
        fetchHistory();
    }, [userId]);
...
const App: React.FC = () => {
    ...
    const [userYearGroup, setUserYearGroup] = useState<string | null>(null);
    ...
    const isGamePage = GAME_PAGES.has(page);
    const { remainingSeconds, isBlocked, cooldownUntil } = useGameTimeLimit(user?.uid, gameTimeLimitMinutes, isGamePage);

    const checkAdmin = async (uid: string): Promise<boolean> => {
        const res = await fetch('/api/check-admin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ uid }) });
    ...
    useEffect(() => {
        const fetchMocks = async () => {
            try { const mocks = await getMocks(); setActiveMocks(mocks.filter(m => m.isActive)); } catch (e) { console.error("Failed to fetch active mocks", e); }
        };
        fetchMocks();
    }, []);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (data.featureToggles) setFeatureFlags(prev => ({ ...prev, ...data.featureToggles }));
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            if (!user) { setUserYearGroup(null); setForceLessonMode(false); setIsLessonMode(false); return; }
            if (user.forcedLessonMode) { setForceLessonMode(true); setIsLessonMode(true); }
            try {
                const classesCol = collection(db, 'classes');
                const q = query(classesCol, where('studentIds', 'array-contains', user.uid));
                const classSnaps = await getDocs(q);
                let foundYear = null, classForced = false;
                if (!classSnaps.empty) {
                ...
                if (classForced) { setForceLessonMode(true); setIsLessonMode(true); } else if (!user.forcedLessonMode) { setForceLessonMode(false); }
            } catch (e) { console.error("Failed to fetch user class data", e); setUserYearGroup(null); }
        };
        fetchUserData();
    ...
    useEffect(() => {
        const unsubscribe = onAuthChange(async (firebaseUser) => {
            if (firebaseUser) {
                ...
                let userLevel: UserLevel | undefined, role = 'student';
                if (!userSnap.exists()) {
                    await setDoc(userRef, { email: firebaseUser.email, displayName: firebaseUser.displayName, createdAt: new Date().toISOString(), role: 'student' });
                ...
                const authUser: AuthUser = { uid: firebaseUser.uid, displayName: firebaseUser.displayName, email: firebaseUser.email, photoURL: firebaseUser.photoURL, level: userLevel, hasSeenTour: userSnap.data()?.hasSeenTour };
                setUser(authUser);
                ...
                setUser(null); setIsAdmin(false); setShowLevelSelector(false);
            }
            setIsLoading(false);
        });
        return () => unsubscribe();
    ...
    useEffect(() => {
        if (!user) return;
        const interval = setInterval(() => logUserActivity(user.uid, 'heartbeat', { page }), 5 * 60 * 1000);
        return () => clearInterval(interval);
    }, [user, page]);

    const handleLevelSelect = async (level: UserLevel) => {
        if (user) { await updateDoc(doc(db, 'users', user.uid), { level }); setUser({ ...user, level }); setShowLevelSelector(false); }
    };

    useEffect(() => { if (user && user.level && !user.hasSeenTour && !showLevelSelector) setShowTour(true); }, [user, showLevelSelector]);

    const handleTourComplete = async () => {
        if (user) { await updateUserTourStatus(user.uid, true); setUser(prev => prev ? ({ ...prev, hasSeenTour: true }) : null); setShowTour(false); }
    };

    const handleReplayTour = () => setShowTour(true);

    const handleNavigate = (newPage: Page, param?: any) => {
        setPage(newPage); setSessionToView(null); setDraftToResume(null);
        if (newPage === 'mock_detail' && param) setSelectedMockId(param);
        if (newPage === 'simulation_view' && param) setSelectedSimulationId(param);
        if (newPage === 'memory_recall_active' && param) setMemoryRecallSessionParams(param);
        if (newPage === 'question_practice' && param) setQuestionPracticeParams(param);
        else if (newPage !== 'question_practice') setQuestionPracticeParams(null);
    };
    ...
    const handleStartFlashcardQuiz = (deck: FlashcardItem[]) => { setFlashcardDeck(deck); setPage('quiz_mode'); };
    const handleViewSession = (session: CompletedSession) => { setSessionToView(session); setDraftToResume(null); setPage('question_practice'); };
    const handleResumeDraft = (draft: DraftSession) => { setDraftToResume(draft); setSessionToView(null); setPage('question_practice'); };
    const handleImpersonate = (impersonatedUser: AuthUser) => { setUser(impersonatedUser); setPage('dashboard'); };
    ...
    if (isLoading) return <div className="flex h-screen items-center justify-center bg-stone-100 dark:bg-stone-950"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div></div>;
    ...
    return (
        <div className={`min-h-screen ${theme.bgPattern} bg-fixed ${theme.bgClass} transition-colors duration-300`}>
            ...
            {page === 'dashboard' && (
                <HubLayout title={`Welcome back, ${user.displayName?.split(' ')[0] || 'Accountant'}`} subtitle={theme.subtitle} gradient={theme.hubGradient} onReplayTutorial={handleReplayTour} isLessonMode={isLessonMode} onToggleLessonMode={setIsLessonMode} forceLessonMode={forceLessonMode}>
                        ...
                        {!isLessonMode && (
                            <section className="animate-fade-in">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    ...
                                    <HubCard icon={<span className="text-4xl">📊</span>} title="My Assessments" description="View all your past assessment marks, teacher feedback, and overall grade profile." onClick={() => handleNavigate('assessment_hub')} shadowColor="shadow-lime-500/20" accentColor="text-lime-600 hover:text-lime-700" />
                                    <HubCard icon={<span className="text-4xl">🧠</span>} title="Memory Recall" description="Use the 'Blurting' technique to solidify your knowledge and track your progress." onClick={() => handleNavigate('memory_recall_hub')} shadowColor="shadow-emerald-500/20" accentColor="text-emerald-600 hover:text-emerald-700" />
                                </div>
                            </section>
                        )}
                        <section className="animate-fade-in [animation-delay:0.1s]">
                            <h2 className="text-2xl font-bold text-stone-700 dark:text-stone-200 mb-6 flex items-center gap-3"><span className="text-3xl">✍️</span> Exam Training Centre</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                <HubCard icon={<span className="text-4xl">📝</span>} title="Question Practice" description="Generate exam-style questions, write answers, and get instant AI marking and feedback." onClick={() => handleNavigate('question_practice_hub')} shadowColor="shadow-blue-500/20" accentColor="text-blue-600 hover:text-blue-700" />
                                <HubCard icon={<span className="text-4xl">🎧</span>} title="Walking Talking Mocks" description="Experience a realistic exam simulation with step-by-step audio guidance and instant marking." onClick={() => handleNavigate('walking_talking_mock')} shadowColor="shadow-indigo-500/20" accentColor="text-indigo-600 hover:text-indigo-700" />
                                <HubCard icon={<span className="text-4xl">🛠️</span>} title="Skills & Structure" description="Master command words, essay structures, and essential maths skills." onClick={() => handleNavigate('skills_practice')} shadowColor="shadow-purple-500/20" accentColor="text-purple-600 hover:text-purple-700" />
                                <HubCard icon={<span className="text-4xl">🚦</span>} title="RAG Analysis" description="Track your mastery across all topics with Red-Amber-Green ratings." onClick={() => handleNavigate('rag_analysis')} shadowColor="shadow-orange-500/20" accentColor="text-orange-600 hover:text-orange-700" disabled={!featureFlags.ragAssessment} />
                                {!isLessonMode && <HubCard icon={<span className="text-4xl">🎯</span>} title="Exams" description="Access targeted revision materials for upcoming exams and mock series." onClick={() => handleNavigate('mocks_hub')} shadowColor="shadow-rose-500/20" accentColor="text-rose-600 hover:text-rose-700" />}
                            </div>
                        </section>
                        <section className="animate-fade-in [animation-delay:0.2s]">
                            <h2 className="text-2xl font-bold text-stone-700 dark:text-stone-200 mb-6 flex items-center gap-3"><span className="text-3xl">🚀</span> Interactive & Future</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {!isLessonMode && (
                                    <>
                                        <HubCard icon={<span className="text-4xl">🏛️</span>} title="Careers & University" description="Explore university courses, find local opportunities, and build your CV." onClick={() => handleNavigate('careers_university')} shadowColor="shadow-amber-500/20" accentColor="text-amber-600 hover:text-amber-700" />
                                        <HubCard icon={<span className="text-4xl">🎤</span>} title="Podcast Studio" description="Create custom audio deep-dives on any topic. Hosted by Alex and Sam." onClick={() => handleNavigate('podcast_studio')} shadowColor="shadow-pink-500/20" accentColor="text-pink-600 hover:text-pink-700" />
                                        <HubCard icon={<span className="text-4xl">🎵</span>} title="Song Generator" description="Turn your revision into catchy tunes using AI." onClick={() => handleNavigate('song_generator')} shadowColor="shadow-violet-500/20" accentColor="text-violet-600 hover:text-violet-700" disabled={!featureFlags.songGenerator} />
                                        <HubCard icon={<span className="text-4xl">🎮</span>} title="Game Zone" description="Test your knowledge with interactive games like Flappy Accounts, Block Blast, and Account Swipe." onClick={() => handleNavigate('games_hub')} shadowColor="shadow-teal-500/20" accentColor="text-teal-600 hover:text-teal-700" />
                                    </>
                                )}
                            </div>
                        </section>
            ...
            {page === 'mock_detail' && selectedMockId && activeMocks.find(m => m.id === selectedMockId) && <MockDetailView user={user} onBack={() => handleNavigate('mocks_hub')} mockId={selectedMockId} mockData={activeMocks.find(m => m.id === selectedMockId)!} />}
            ...
            {page === 'games_hub' && <GamesHubView onNavigate={handleNavigate} onStartGame={handleStartGame} user={user} featureFlags={featureFlags} remainingSeconds={remainingSeconds} isBlocked={isBlocked} cooldownUntil={cooldownUntil} />}
            ...
            <GameTimerBanner remainingSeconds={remainingSeconds} isVisible={isGamePage} />
        </div>
    );
};
